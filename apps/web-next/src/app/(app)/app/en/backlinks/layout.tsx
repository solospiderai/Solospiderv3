...
export default function BacklinksLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#0e0c1a] text-slate-900 dark:text-white flex flex-col font-sans -m-6 p-6">
      {/* SoloSpider Top Navigation Header */}
      <header className="border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#18162c] sticky top-0 z-40 backdrop-blur-md rounded-2xl mb-6 shadow-sm">
          ...
          <div className="flex items-center justify-between h-16 gap-3">
            <div className="flex items-center gap-2.5 shrink-0">
              ...
              <div className="shrink-0">
                <h1 className="text-sm sm:text-base font-bold text-slate-900 dark:text-white leading-none whitespace-nowrap">SoloSpider Backlinks</h1>
                <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1 whitespace-nowrap hidden sm:block">AI Backlink Acquisition & GSC Sync</p>
              </div>
            </div>

            {/* Sub-navigation Links */}
            <nav className="hidden md:flex items-center gap-1 overflow-x-auto py-1 max-w-full">
              {navItems.map((item) => {
            ...
            <Link
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 text-xs rounded-xl border border-slate-200 dark:border-slate-700 font-medium transition shrink-0"
            >
              <Plug2 className="w-3.5 h-3.5 text-blue-600 dark:text-blue-400" />
              <span className="hidden xl:inline">GSC Integration</span>
            </Link>
          </div>
        </div>

        {/* Mobile Nav Scrollbar */}
        <div className="md:hidden flex overflow-x-auto border-t border-slate-100 dark:border-slate-800 px-4 py-2 gap-2 text-xs">
          {navItems.map((item) => {
            const Icon = item.icon;
            const isActive = pathname === item.href;
            return (
              <Link
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg whitespace-nowrap ${
                  isActive ? 'bg-blue-600 text-white' : 'text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-800'
                }`}
              >
