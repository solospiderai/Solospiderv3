...
import { Mail, CheckCircle2, AlertCircle, Linkedin, Twitter, Loader2, Users } from 'lucide-react';
      ...
      try {
        ...
        if (bProj) {
          const { data: cList } = await supabase
            .from('contacts')
            .select('*, prospects!inner(backlink_project_id, domain)')
            .eq('prospects.backlink_project_id', bProj.id);
          ...
          if (cList) {
            setContacts(
              cList.map((c: any) => ({
                ...
                role: c.role || 'Editor',
                email: c.email || null,
                domain: c.prospects?.domain || 'Website',
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
        {loading ? (
          ...
          <div className="p-12 text-center text-xs text-slate-500 space-y-3">
            ...
            <p className="font-semibold text-slate-800">No contacts extracted yet.</p>
            <p>Contacts will automatically be extracted and verified when prospect websites are discovered.</p>
          </div>
        )}
      </div>
    </div>
