...
function cleanJsonResponse(rawText: string) {
  ...
  if (suggestedValue === "null") suggestedValue = null;
  
  if (!suggestedValue) {
    if (cleaned.includes("<h1>")) {
      const match = cleaned.match(/<h1>([^]*?)<\/h1>/i);
      if (match) suggestedValue = match[1];
    } else if (cleaned.includes("<title>")) {
      const match = cleaned.match(/<title>([^]*?)<\/title>/i);
      if (match) suggestedValue = match[1];
    }
  }
