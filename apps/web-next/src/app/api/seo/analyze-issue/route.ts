...
function cleanJsonResponse(rawText: string, issueId: string, url: string) {
  ...
  try {
    ...
    if (parsed) {
      if (parsed.suggestedValue === "null" || parsed.suggestedValue === null) {
        parsed.suggestedValue = null;
      }
      if (!parsed.suggestedValue) {
        parsed.suggestedValue = fallbackSuggestedValue(issueId, cleaned, parsed.recommendation, url);
      }
      return parsed;
    }
  } catch (e) {
  ...
  try {
    ...
    if (parsed) {
      ...
      if (!parsed.suggestedValue) {
        parsed.suggestedValue = fallbackSuggestedValue(issueId, repaired, parsed.recommendation, url);
      }
      return parsed;
    }
  } catch (e) {
  ...
  console.log("[cleanJsonResponse] Standard and repaired parses failed, running regex fallback...");
  const recommendation = extractField(cleaned, "recommendation") || cleaned;
  let codeSnippet = extractField(cleaned, "codeSnippet") || undefined;
  if (codeSnippet === "null") codeSnippet = undefined;
  
  const explanation = extractField(cleaned, "explanation") || "Optimized recommendation generated by AI.";
  
  let suggestedValue = extractField(cleaned, "suggestedValue");
  if (suggestedValue === "null" || suggestedValue === null) suggestedValue = null;
  
  if (!suggestedValue) {
    suggestedValue = fallbackSuggestedValue(issueId, cleaned, recommendation, url) || null;
  }
...
function fallbackSuggestedValue(issueId: string, rawText: string, recommendation: string, url: string): string | null {
  if (rawText.includes("<h1>")) {
    const match = rawText.match(/<h1>([^]*?)<\/h1>/i);
  ...
  if (issueId === "thin-content") {
    return recommendation || "Expanded content";
  }
  if (issueId === "broken-links") {
    return "/";
  }
  if (issueId === "no-sitemap") {
    let domain = "solospider.ai";
    try {
  ...
  if (issueId === "missing-schema") {
    if (rawText.includes("@context")) {
      const match = rawText.match(/(\{[\s\S]*?"@context"[\s\S]*?\})/);
      if (match) return match[1];
    }
    return "WebPage";
  }
...
function extractField(jsonStr: string, fieldName: string): string | null {
  ...
  const nullRegex = new RegExp(`"${fieldName}"\\s*:\\s*(null|true|false|\\d+)`, "i");
  const nullMatch = jsonStr.match(nullRegex);
  if (nullMatch) {
    return nullMatch[1];
  }
  const lastRegex = new RegExp(`"${fieldName}"\\s*:\\s*"([^]*?)$`, "i");
  const lastMatch = jsonStr.match(lastRegex);
  if (lastMatch) {
    return lastMatch[1].replace(/"\s*\}?$/, "");
  }
  return null;
}
...
JSON Schema:
{
  ...
  "suggestedValue": "Strictly the plain-text value to apply for fixing the issue. For missing/duplicate/long titles: the suggested title string. For missing/duplicate descriptions: the suggested meta description string. For missing H1: the suggested H1 string. For schema: the schema type (e.g. WebPage, Article, etc.). Return null if not applicable."
    ...
    if (rawContent) {
      try {
        const parsed = cleanJsonResponse(rawContent, issueId, url);
